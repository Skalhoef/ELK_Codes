...
!  0 : no gradients
!  1 : gradients required for GGA with no post-processing: |grad rho|,
!      grad^2 rho, (grad rho).(grad |grad rho|)
!  2 : gradients required for GGA with post-processing: |grad rho|^2
!  3 : as 2 but with the laplacian, grad^2 rho
!  4 : as 2 but with the kinetic energy density, tau
!  5 : as 4 but with the laplacian, grad^2 rho
integer xcgrad
...
real(8), pointer, contiguous :: vsmt(:,:),vsir(:)
...
real(8), pointer, contiguous :: bsir(:,:)
! G-space interstitial Kohn-Sham effective potential
complex(8), allocatable :: vsig(:)
! trimvg is .true. if the components of the effective potential are to be set to
! zero for |G| > 4 gkmax before the characteristic function is applied
logical trimvg,trimvg0
...
logical tscissor
! scissor correction applied to eigenvalues and momentum matric elements
real(8) scissor
...
real(8) hybridc
! tolerance for convergence of the absolute change in residual
real(8) epsdres
...
!---------------------------------!
! code version
integer, parameter :: version(3)=[9,5,14]
