...
integer ngp2,igp,i0,i,j,k,nthd
real(8) a1,a2,a3,t1
real(8) ts0,ts1
complex(8) z1
complex(4) c1
...
! allocatable arrays
complex(4), allocatable :: wfmt0(:,:),wfgp0(:,:),wfir1(:),wfir2(:)
...
!$OMP PRIVATE(irc,t1,i0,i,j,k,l,nm,lm,z1) &
...
!$OMP PRIVATE(ist,jst,igp,t1,c1,i,j,k) &
!$OMP NUM_THREADS(nthd)
...
!$OMP DO
  do jst=1,nstfv
    wfir1(:)=0.e0
    do igp=1,ngp
      wfir1(igfft(igpig(igp)))=wfgp0(igp,jst)
    end do
...
! multiply with magnetic field and transform to G-space
    if (spinpol) then
      ...
      do igp=1,ngp
        wfgp1(igp)=wfir2(igfft(igpig(igp)))
      end do
      ...
      if (ncmag) then
        ...
        call cfftifc(3,ngridg,-1,wfir2)
        do igp=1,ngp
          wfgp3(igp)=wfir2(igfft(igpig(igp)))
        end do
      end if
    else
      wfgp1(1:ngp)=0.e0
...
! apply vector potential if required
    if (tafield) then
      ...
      do igp=1,ngp
        ...
        wfir1(igfft(igpig(igp)))=t1*wfgp0(igp,jst)
      end do
      call cfftifc(3,ngridg,1,wfir1)
      wfir1(1:ngtot)=wfir1(1:ngtot)*cfunir(1:ngtot)
      call cfftifc(3,ngridg,-1,wfir1)
      do igp=1,ngp
        c1=wfir1(igfft(igpig(igp)))
...
! spins not coupled: block diagonalise H
  ...
  do j=1,nstfv
    evecsv(nstfv+1:nstsv,j)=0.d0
    evecsv(1:nstfv,nstfv+j)=0.d0
  end do
end if
call timesec(ts1)
!$OMP ATOMIC
