...
! check if task can be run with MPI
  if (lp_mpi > 0) then
    if (any(task == [0,1,2,3,5,15,16,28,29,61,62,63,110,120,121,125,135,136, &
     162,170,180,185,188,200,201,205,208,240,241,270,271,300,320,330,331,350, &
     351,371,372,373,380,390,420,421,440,460,461,462,463,471,478,600,610,620, &
     640,700,701])) then
      continue
    else
...
!BOI
! !TITLE: {\huge{\sc The Elk Code Manual}}\\ \Large{\sc Version 9.5.14}\\ \vskip 20pt \includegraphics[height=1cm]{elk_silhouette.pdf}
...
!   Yunfan Liang, Xavier Gonze, Mike Bruckhoff, Eddie Harris-Lee and Andreas
!   Fischer. Special mention of David Singh's very useful book on the LAPW
...
!
!   \vspace{12pt}
!   Berlin, Halle, Jerusalem and Uppsala, April 2024
!   \newpage
!
!   \section{Units}
...
!   compile Libxc version 6. This should have produced the files {\tt libxc.a}
...
!   $$ A\rightarrow (1+\delta t\,e_k)A, $$
!   where $\delta t$ is an infinitesimal equal in practice to {\tt deltast} and
!   $e_k$ is the $k^{\rm th}$ strain tensor. Numerical finite differences are
!   used to compute the stress tensor as the derivative of the total energy
!   $dE_k/dt$.
!
!   \block{dft+u}{
...
!
!   \block{mixtype}{
!   {\tt mixtype } & type of mixing required for the potential & integer & 3}
!   Currently implemented are:
!   \vskip 6pt
!   \begin{tabularx}{\textwidth}[h]{lX}
!   0 & Linear mixing \\
...
!
!   \block{mrmtav}{
!   {\tt mrmtav} & order of averaging applied to the muffin-tin radii &
!    integer & 0}
!   Crystal structures with muffin-tin radii which are widely varying in size
!   can cause calculations to become unstable. Applying a simple averaging
!   procedure to the radii reduces this variation and can improve stability.
!   The larger {\tt mrmtav}, the more equal the muffin-tin radii will become.
!   See the routine {\tt rmtavrg}.
!
!   \block{mstar}{
...
!
!   \block{num\_wann}{
!   {\tt num\_wann} & number of Wannier90 wavefunctions & integer & 0}
!   If ${\tt num\_wann}>0$ then this is the number of Wannier wavefunctions
!   to be found by the Wannier90 package. If ${\tt num\_wann}\le 0$ then the
!   number of wavefunctions is given by ${\tt num\_bands}+{\tt num\_wann}$.
!
!   \block{nvbse}{
...
!   occupation numbers of the Kohn-Sham states. The variable {\tt swidth}
!   determines the width of the approximate delta function. Currently
!   implemented are
!   \vskip 6pt
!   \begin{tabularx}{\textwidth}[h]{lX}
!   0 & Gaussian \\
...
!        code to calculate occupation numbers. \\
...
!
!   \block{tau0oep}{
!   {\tt tau0oep} & initial step length for the OEP iterative solver & real &
!    $0.5$}
!   The optimised effective potential is determined using an interative method
!   [Phys. Rev. Lett. 98, 196405 (2007)]. This variable sets the step length as
!   described in the article. See {\tt maxitoep}.
!
!   \block{taufsm}{
