...
! compute v^1/2 chi0 v^1/2
  call genvchi0(.true.,ik,lock,vecql,gclgq,jlgqr,ylmgq,sfacgq,nm,vchi0)
end do
!$OMP END PARALLEL DO
call freethd(nthd)
...
! calculate symmetric epsilon_0 = 1 - v^1/2 chi0 v^1/2
eps0(:,:,:)=-vchi0(:,:,:)
do i=1,nm
  eps0(i,i,:)=eps0(i,i,:)+1.d0
end do
! initialise epsilon for use with the bootstrap functional
...
! self-consistent bootstrap f_xc
  ...
  if (mod(it,10) == 0) then
    ...
    t1=dble(vfxc(1,1,1))
    write(*,'(" head of matrix v^(-1/2) f_xc v^(-1/2) : ",G18.10)') t1
