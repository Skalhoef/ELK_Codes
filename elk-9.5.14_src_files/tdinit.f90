...
tfav0=.false.
! ensure eigenvectors are written to disk during initialisation
wrtdsk0=wrtdsk
wrtdsk=.true.
...
if (tatdisp) call readforcet
! read the atomic displacements and velocities
if (trdatdv) call readatdvc
wrtdsk=wrtdsk0
! make the time-dependent Kohn-Sham orbitals strictly orthogonal after each
! time step for complex time evolution
if (tdphi /= 0.d0) ntsorth=1
