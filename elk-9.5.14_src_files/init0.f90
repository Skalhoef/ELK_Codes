...
if (any(task == [5,10,170,300,460,461,462,463,600,620,700,701]).or.ksgwrho) then
  tevecsv=.true.
end if
...
if (any(xcgrad == [3,4,5]).and.ncmag) then
  write(*,*)
...
if (reducebf < 1.d0-1.d-4) then
  if (all(task /= [0,1,2,3,5,28,200,201,350,351])) then
    bfieldc(:)=0.d0
    bfcmt(:,:,:)=0.d0
  end if
end if
...
if (dftu /= 0) then
  if (any(task == [5,300,600,610,620,640])) then
    write(*,*)
