...
!   where $n_i$ are the occupation numbers and $\epsilon_i$ are the eigenvalues
!   of both the core and valence states; $\rho$ is the density; ${\bf m}$ is the
...
! add vector potential constant term (1/2c²)A²Q if required
if (tafield) then
  sm=sum(afieldc(:)**2)
  engykn=engykn+(0.5d0/solsc**2)*sm*chgval
end if
!-------------------------------!
